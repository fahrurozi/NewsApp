    ...
    <div class="row">
        ...
        <form method="post" action="{{route('events.update', $events->id)}}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" id="name" class="form-control" value="{{$events->name}}" placeholder="Masukan Judul Buku">
            </div>
            <div class="form-group">
                <label for="penulis">Description</label>
                <textarea name="description" id="description" class="form-control" cols="30" rows="10">{{$events->description}}</textarea>
            </div>
            <div class="form-group">
                <label for="penulis">Date</label>
                <input type="text" name="date" value="{{$events->date->format('d/m/Y')}}" id="date" class="date form-control" value="{{$events->name}}" placeholder="masukan tanggal">
            </div>
            <div class="">
